...
class TestPaiTask(unittest.TestCase):
    ...
    def test_success(self):
        ...
        responses.add(responses.POST, 'http://127.0.0.1:9186/api/v1/jobs',
                      json={"message": f"update job {sk_task.name} successfully"}, status=202)

        responses.add(responses.GET, f'http://127.0.0.1:9186/api/v1/jobs/{sk_task.name}',
                      json={}, status=404)
    ...
    def test_fail(self):
        ...
        responses.add(responses.POST, 'http://127.0.0.1:9186/api/v1/jobs',
                      json={"message": f"update job {fail_task.name} successfully"}, status=202)

        responses.add(responses.GET, f'http://127.0.0.1:9186/api/v1/jobs/{fail_task.name}',
                      json={}, status=404)
