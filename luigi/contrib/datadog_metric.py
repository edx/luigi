...
class DatadogMetricsCollector(MetricsCollector):
    ...
    def handle_task_started(self, task):
        ...
        text = f"A task has been started in the pipeline named: {task.family}"
        tags = [f"task_name:{task.family}"] + self._format_task_params_to_tags(task)
    ...
    def handle_task_failed(self, task):
        ...
        text = f"A task has failed in the pipeline named: {task.family}"
        tags = [f"task_name:{task.family}"] + self._format_task_params_to_tags(task)
    ...
    def handle_task_done(self, task):
        ...
        text = f"A task has completed in the pipeline named: {task.family}"
        tags = [f"task_name:{task.family}"] + self._format_task_params_to_tags(task)
    ...
    def default_tags(self):
        ...
        env_tag = f"environment:{self._config.environment}"
