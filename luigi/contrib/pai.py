...
class PaiTask(luigi.Task):
    ...
    def output_dir(self):
        ...
        return f'$PAI_DEFAULT_FS_URI/{self.name}/output'
    ...
    def __init_token(self):
        ...
        logger.debug(f'Get token request {request_json}')
        response = rs.post(urljoin(self.__openpai.pai_url, '/api/v1/token'),
                           headers={'Content-Type': 'application/json'}, data=request_json)
        logger.debug(f'Get token response {response.text}')
        if response.status_code != 200:
            msg = f'Get token request failed, response is {response.text}'
            logger.error(msg)
            raise Exception(msg)
        else:
    ...
    def __check_job_status(self):
        response = rs.get(urljoin(self.__openpai.pai_url, f'/api/v1/jobs/{self.name}'))
        logger.debug(f'Check job response {response.text}')
        if response.status_code == 404:
            msg = f'Job {self.name} is not found'
            logger.debug(msg)
            raise HTTPError(msg, response=response)
        elif response.status_code != 200:
            msg = f'Get job request failed, response is {response.text}'
            logger.error(msg)
            raise HTTPError(msg, response=response)
        job_state = response.json()['jobStatus']['state']
        if job_state in ['UNKNOWN', 'WAITING', 'RUNNING']:
            logger.debug(f'Job {self.name} is running in state {job_state}')
            return False
        else:
            msg = f'Job {self.name} finished in state {job_state}'
            logger.info(msg)
    ...
    def run(self):
        ...
        logger.debug(f'Submit job request {request_json}')
        response = rs.post(urljoin(self.__openpai.pai_url, '/api/v1/jobs'),
                           headers={'Content-Type': 'application/json',
                                    'Authorization': f'Bearer {self.__token}'}, data=request_json)
        logger.debug(f'Submit job response {response.text}')
        # 202 is success for job submission, see https://github.com/Microsoft/pai/blob/master/docs/rest-server/API.md
        if response.status_code != 202:
            msg = f'Submit job failed, response code is {response.status_code}, body is {response.text}'
            logger.error(msg)
            raise HTTPError(msg, response=response)
